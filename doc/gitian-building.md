...
*Setup instructions for a Gitian build of Pearlcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pearlcoin
...
- [Building Pearlcoin Core](#building-pearlcoin-core)
...
Clone the git repositories for pearlcoin and Gitian.
...
in the pearlcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Pearlcoin Core
----------------

To build Pearlcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the pearlcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pearlcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/pearlcoin-project/pearlcoin
...
For example:
```bash
URL=https://github.com/thrasher-/pearlcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pearlcoin=${COMMIT} --url pearlcoin=${URL} ../pearlcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pearlcoin=${COMMIT} --url pearlcoin=${URL} ../pearlcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pearlcoin=${COMMIT} --url pearlcoin=${URL} ../pearlcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pearlcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pearlcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pearlcoin
SIGPATH=/some/root/path/pearlcoin-detached-sigs

./bin/gbuild --url pearlcoin=${BTCPATH},signature=${SIGPATH} ../pearlcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.PRC` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pearlcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pearlcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pearlcoin-osx-build.assert
```
...
[gitian.sigs.PRC](https://github.com/pearlcoin-project/gitian.sigs.PRC/) repository, or if that's not possible create a pull
